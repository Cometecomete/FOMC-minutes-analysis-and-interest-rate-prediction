...
df_tfidf_lsa = Normalizer(copy=False).fit_transform(df_tfidf_lsa)
df_tfidf_lsa = pd.DataFrame(df_tfidf_lsa,  columns = ["component%d" % i for i in range(1,401)])

minutes_tfidf_lsa = pd.concat([minutes,df_tfidf_lsa],axis = 1)
minutes_tfidf_lsa['oldDate'] =pd.to_datetime(minutes_tfidf_lsa['file_name'],format='%Y%m%d',errors='ignore')

minutes_tfidf_lsa['Date'] = minutes_tfidf_lsa['oldDate'] + datetime.timedelta(days=23)
tfIdf_IR_lsa = pd.merge(IR,minutes_tfidf_lsa,on = 'Date',how = 'left')
tfIdf_IR_diff_lsa = tfIdf_IR_lsa.dropna()
tfIdf_IR_diff_lsa['rateChange'] = tfIdf_IR_diff_lsa['WILLREITIND'].shift(-1) - tfIdf_IR_diff_lsa['WILLREITIND']
tfIdf_IR_diff_lsa = tfIdf_IR_diff_lsa.dropna()
tfIdf_IR_diff_lsa=tfIdf_IR_diff_lsa.drop(['Unnamed: 0','Index','year','month','day','file_name','oldDate','content'],axis=1)
CorTfidfIR_lsa = CorTerms(tfIdf_IR_diff_lsa.columns[3:-1],tfIdf_IR_diff_lsa,tfIdf_IR_diff_lsa['rateChange'],top = 20,bottom = 20)
